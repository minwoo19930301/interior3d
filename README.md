...
- 프로젝트 JSON 내려받기·검증 후 불러오기, 브라우저 자동 저장·복구
...
상단 **프로젝트**에서 방 구조·가구 배치·표시 단위를 JSON 파일로 내려받거나 다른 기기에서 열 수 있습니다. 파일을 선택하면 파일명과 오브젝트 수를 먼저 보여주고, **이 프로젝트로 교체**를 눌러 적용합니다. 교체와 자동 저장본 복구는 한 번의 되돌리기로 이전 장면·선택·단위까지 복구합니다. 단위 변경도 되돌리기 대상입니다.

편집 내용은 0.5초 뒤 이 브라우저에 자동 저장되며, 탭을 숨기거나 떠날 때 대기 중인 변경을 저장합니다. 공유 링크 없이 다시 접속하면 마지막 저장본을 복구합니다. 공유 링크의 장면(비어 있는 장면 포함)이 우선이고, 처음 열기만 해서는 기존 저장본을 덮어쓰지 않습니다. **자동 저장본 복구**로 공유 링크를 열기 전의 작업을 불러올 수도 있습니다. 한 브라우저/사이트에 최신 저장본 하나만 보관하므로, 여러 버전과 기기 간 이동에는 JSON 파일을 사용하세요.

파일은 버전 1의 Interior3D 형식이며 최대 2 MiB / 오브젝트 2,000개를 지원합니다. 잘못된 파일, 알 수 없는 가구 종류, 지원하지 않는 버전은 현재 작업을 바꾸기 전에 거부합니다. 카메라 위치, 복사한 가구와 되돌리기 이력은 파일에 포함하지 않습니다. 브라우저 저장 권한이나 용량 문제는 프로젝트 버튼의 `!` 및 프로젝트 창에 표시하며, 마지막으로 저장에 성공한 데이터와 현재 편집 내용은 유지합니다.
...
프로젝트 회귀 검사는 파일 왕복·크기/스키마 검증, 원자적인 불러오기/되돌리기, 공유 링크 우선순위, 빈 저장본 복구, 저장 실패 보존과 마지막 변경의 저장을 확인합니다.
...
`node tests/project-browser.mjs`는 1440/390px에서 실제 파일 다운로드·불러오기, 되돌리기/다시 실행, 잘못된 파일 거부, 자동 저장 복구와 공유 링크 우선순위, 저장 공간 오류 시 파일 백업을 확인합니다.
`PLAYWRIGHT_MODULE`로 기존 Playwright 모듈 경로를, `TEST_URL`로 로컬 주소를,
`EVIDENCE_DIR`로 스크린샷 저장 폴더를 지정할 수 있습니다.

2026-09-07 프로젝트 저장 기능 검증: 126개 회귀 테스트, lint, 정적 빌드가 통과했습니다.
`tests/project-browser.mjs`도 실제 Chrome의 1440/390px 화면에서 통과했습니다.
파일 다운로드·미리보기·교체, 단위까지 되돌리기/다시 실행, 잘못된 파일 거부,
공유 링크 우선·빈 장면 복구·페이지를 떠날 때 저장, 저장 용량 오류 안내와 파일 백업을 확인했습니다.
모바일 폭 검사는 물리적 휴대폰 시험과 구분합니다. GitHub CI는 설치·테스트·lint·빌드만 수행하며 배포하지 않습니다.
